...
function createNode(type, x, y) {
    switch (type) {
      ...
      case types.nand:
        return new NAND(x, y);
      case types.nand3:
        return new NAND_3(x, y);
      case types.nor:
        return new NOR(x, y);
      case types.nor3:
        return new NOR_3(x, y);
      case types.xor:
        return new XOR(x, y);
      case types.xnor:
        return new XNOR(x, y);
      case types.srlatch:
        return new SR_LATCH(x, y);
      case types.dlatch:
        return new D_LATCH(x, y);
      case types.dflipflop:
        return new D_FLIPFLOP(x, y);
      case types.jkflipflop:
        return new JK_FLIPFLOP(x, y);
      case types.register4:
        return new REGISTER_4(x, y);
      case types.display:
        return new DISPLAY(x, y);
      case types.displayDriver:
        return new DISPLAY_DRIVER(x, y);
      case types.junction:
        return new JUNCTION(x, y);
      case types.pulse:
        return new PULSE(x, y);
      case types.negedgepulse:
        return new NEG_EDGE_PULSE(x, y);
      case types.posedgepulse:
        return new POS_EDGE_PULSE(x, y);
      case types.multiplexer: // Add the case for Multiplexer
        return new MULTIPLEXER(x, y);
      default:
        return null;
    }
  }
...
function createBG(){
    ...
    bg.rotate(-PI / 16)
    bg.text("B.T.K.I.T DWARAHAT", 0, 0);
    return bg;
}
...
function mouseClicked() {
    ...
    for (const node of nodes) {
        if (node.contains(mouseX, mouseY)) {
            ...
            break; // Stop checking after the first clicked node
        }
    }
}
